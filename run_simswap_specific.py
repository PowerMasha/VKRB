...
def video_swap(video_path, id_vetor, specific_person_id_nonorm, id_thres, 
               swap_model, detect_model, faceenhancer, 
               save_path, temp_results_dir='/home/output/results', 
               crop_size=224, 
               use_mask = True):
        ...
        if  ret:
            ...
            if detect_results is not None:
                ...
                if min_value < id_thres:
                    ...
                    reverse2wholeimage([frame_align_crop_tenor_list[min_index]], [final_result], [frame_mat_list[min_index]], crop_size, frame,\
...
def main():
    ...
    app.prepare(ctx_id= 0, det_thresh=0.4, det_size=(512,512))
    with torch.no_grad():
        ...
        specific_person_downsample = F.interpolate(specific_person, scale_factor=0.1)
        specific_person_id_nonorm = simswap_model.netArc(specific_person_downsample)

        video_swap(opt.video_path, latend_id, specific_person_id_nonorm, opt.id_thres,
                   simswap_model, app, faceenhancer, opt.output_path, temp_results_dir=opt.temp_path, use_mask=opt.use_mask)
